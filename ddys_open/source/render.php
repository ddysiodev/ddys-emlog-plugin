...
function ddys_open_wrap($html, $args = array())
{
    ...
    return '<div class="ddys-emlog ddys-emlog-theme-' . ddys_open_attr($theme) . ' ddys-emlog-layout-' . ddys_open_attr($layout) . '" style="--ddys-emlog-columns:' . $columns . '">' . $html . '</div>';
}
...
function ddys_open_render_error($payload, $args = array())
{
    ...
    return ddys_open_wrap('<div class="ddys-emlog-alert ddys-emlog-alert-error">' . ddys_open_h($message) . '</div>', $args);
}

function ddys_open_render_empty($message, $args = array())
{
    return ddys_open_wrap('<div class="ddys-emlog-empty">' . ddys_open_h($message) . '</div>', $args);
}
...
function ddys_open_render_card($item, $settings)
{
    ...
    $html = '<article class="ddys-emlog-card">';
    if ($poster !== '') {
        $html .= '<div class="ddys-emlog-poster"><img src="' . ddys_open_attr($poster) . '" alt="' . ddys_open_attr($title) . '" loading="lazy"></div>';
    }
    $html .= '<div class="ddys-emlog-card-body">';
    $html .= '<h3 class="ddys-emlog-card-title">';
    ...
    $html .= '</h3>';
    if (!empty($meta)) {
        $html .= '<div class="ddys-emlog-meta">' . ddys_open_h(implode(' / ', $meta)) . '</div>';
    }
    if ($summary !== '') {
        $html .= '<div class="ddys-emlog-summary">' . ddys_open_h(ddys_open_substr(strip_tags((string)$summary), 0, 150)) . '</div>';
    }
    $html .= '</div></article>';
    return $html;
}
...
function ddys_open_render_list($payload, $args = array())
{
    ...
    $settings = ddys_open_settings();
    $html = '<div class="ddys-emlog-items">';
...
function ddys_open_render_sources($payload, $args = array(), $inner = false)
{
    ...
    $html = '<div class="ddys-emlog-sources">';
    foreach ($groups as $name => $resources) {
        if (!is_array($resources)) {
            continue;
        }
        $html .= '<section class="ddys-emlog-source-group"><h3>' . ddys_open_h($name) . '</h3>';
        foreach ($resources as $resource) {
            ...
            $html .= '<p class="ddys-emlog-resource">' . ddys_open_render_resource_links($title, $url) . '</p>';
        }
        $html .= '</section>';
    }
    $html .= '</div>';
...
function ddys_open_render_calendar($payload, $args = array())
{
    ...
    $settings = ddys_open_settings();
    $html = '<div class="ddys-emlog-calendar">';
    foreach ($days as $day => $items) {
        if (is_array($items) && isset($items['shows']) && is_array($items['shows'])) {
            $items = $items['shows'];
        }
        $html .= '<section class="ddys-emlog-calendar-day"><h3>' . ddys_open_h($day) . '</h3><div class="ddys-emlog-items">';
        if (is_array($items)) {
            foreach ($items as $item) {
                $html .= ddys_open_render_card($item, $settings);
            }
...
function ddys_open_render_dictionary($payload, $args = array())
{
    ...
    $html = '<div class="ddys-emlog-tags">';
    foreach ($items as $item) {
...
function ddys_open_render_request_form($args = array())
{
    ...
    $html = '<form class="ddys-emlog-request-form" method="post" action="' . ddys_open_attr(ddys_open_endpoint_url('request')) . '" data-ddys-emlog-request-form>';
    ...
    $html .= '<button type="submit">提交求片</button><p class="ddys-emlog-status" role="status"></p></form>';
    return ddys_open_wrap($html, $args);
}
...
function ddys_open_render_pagination_meta($meta)
{
    ...
    return '<div class="ddys-emlog-page-meta">第 ' . ddys_open_h($page) . ' 页，共 ' . ddys_open_h($meta['total']) . ' 条</div>';
}
...
function ddys_open_frontend_assets()
{
    ...
    if (!empty($settings['enable_styles'])) {
        $html .= "\n" . '<link rel="stylesheet" href="' . ddys_open_attr(ddys_open_plugin_url('static/css/frontend.css?v=' . DDYS_OPEN_EMLOG_VERSION)) . '">';
    }
    $html .= "\n" . '<script defer src="' . ddys_open_attr(ddys_open_plugin_url('static/js/frontend.js?v=' . DDYS_OPEN_EMLOG_VERSION)) . '"></script>';
    return $html;
}
...
function ddys_open_page_tabs($active)
{
    ...
    $html = '<nav class="ddys-emlog-tabs">';
