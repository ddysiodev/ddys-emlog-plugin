...
function ddys_open_api_request($method, $path, $params, $body, $options)
{
    ...
    $headers = array(
        ...
        'User-Agent: ddys-emlog-plugin/' . DDYS_OPEN_EMLOG_VERSION
    );
...
function ddys_open_http_request($method, $url, $body, $headers, $timeout)
{
    ...
    if (!ini_get('allow_url_fopen')) {
        return ddys_open_error('当前 PHP 环境未启用 cURL，也未开启 allow_url_fopen。', 502);
    }
    $opts = array(
        'http' => array(
            'method' => $method,
...
function ddys_open_handle_request_form()
{
    ...
    if (!ddys_open_verify_nonce(ddys_open_post('ddys_nonce'), 'front')) {
